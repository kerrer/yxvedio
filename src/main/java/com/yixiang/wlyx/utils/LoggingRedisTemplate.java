...
/**
 * An extension of RedisTemplate that logs exceptions instead of letting them propagate. If the
 * Redis server is unavailable, cache operations are always a "miss" and data is fetched from the
 * database.
 */
public class LoggingRedisTemplate<K, V> extends RedisTemplate<K, V> {
  ...
  public <T> T execute(final RedisCallback<T> action, final boolean exposeConnection,
    ...
    try {
      ...
      logger.warn("Error executing cache operation1: {}", t.getMessage());
      t.printStackTrace();
      return null;
    }
  }
  ...
  public <T> T execute(final RedisScript<T> script, final List<K> keys, final Object... args) {
    try {
      ...
      logger.warn("Error executing cache operation2: {}", t.getMessage());
      return null;
    }
  }
  ...
  public <T> T execute(final RedisScript<T> script, final RedisSerializer<?> argsSerializer,
    ...
    try {
      ...
      logger.warn("Error executing cache operation3: {}", t.getMessage());
      return null;
    }
  }
  ...
  public <T> T execute(final SessionCallback<T> session) {
    try {
      ...
      logger.warn("Error executing cache operation4: {}", t.getMessage());
      return null;
    }
  }
}