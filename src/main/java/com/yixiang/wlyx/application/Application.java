...
import com.yixiang.wlyx.model.Role;
import com.yixiang.wlyx.model.User;
import com.yixiang.wlyx.utils.CommonUtility;
 ...
 * 
 */
public class Application extends MultiThreadedApplicationAdapter implements IPendingServiceCall,
   ...
   * (non-Javadoc)
   * 
   * @see org.red5.server.adapter.MultiThreadedApplicationAdapter#connect(org.red5
    ...
    try {
      ...
      if (ScopeUtils.isRoom(scope)) {
        if (params.length > 1) {
          ...
          if (userid.equals(DemoConstants.GUEST)) {
            LOG.debug("No User for the user [" + userid + "], Connection name [" + connectionName
                + "]");
      ...
      if (demoUser != null && connectionName != null) {
        if (connectionName.equals(DemoConstants.CONNECTIONS.get(0))) {
          ...
          ServiceUtils.invokeOnConnection(connection, "updateUser", new Object[] { demoUser });

          LOG.debug("Public Connection ID for the user [" + userid + "] is ["
              + userConnection.getPublicId() + "]");

        } else if (connectionName.equals(DemoConstants.CONNECTIONS.get(1))) {
          /* For video connection */
          userConnection.setVideoId(connection.getClient().getId());
          userConnection.setVideoConnection(connection);
        } else if (connectionName.equals(DemoConstants.CONNECTIONS.get(2))) {
        ...
        client.setAttribute("userId", demoUser != null ? demoUser.getId() : "");
      }
    } catch (Exception ex) {
   ...
   * 
   * @see
   * org.red5.server.adapter.MultiThreadedApplicationAdapter#streamPublishStart(org.red5.server.
   * api.stream.IBroadcastStream)
   */
  @Override
   ...
   * (non-Javadoc)
   * 
   * @see org.red5.server.adapter.MultiThreadedApplicationAdapter#streamBroadcastStart
   ...
   * (non-Javadoc)
   * 
   * @see org.red5.server.adapter.MultiThreadedApplicationAdapter#streamBroadcastClose
   * (org.red5.server.api.stream.IBroadcastStream)
   */
  @Override
   ...
   * (non-Javadoc)
   * 
   * @see org.red5.server.adapter.MultiThreadedApplicationAdapter#appStart(org.
   ...
   * (non-Javadoc)
   * 
   * @see org.red5.server.api.service.IPendingServiceCall#registerCallback(org.
   ...
   * (non-Javadoc)
   * 
   * @see org.red5.server.api.service.IPendingServiceCall#unregisterCallback(org
   * .red5.server.api.service.IPendingServiceCallback)
   */
  @Override
